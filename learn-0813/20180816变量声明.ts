...
function funUp(shouldInitialize: boolean) {
  if (shouldInitialize) {
    var x = 10;
  }
...
function r(condition: boolean, x: any) {
  if (condition) {
    let x = 100;
    return console.log('innner', x)
  }
...
let input = ['first', 'secound'];

let [firstCount, secoundCount] = input; // firstCount = input[0]; secoundCount = input[1];

[firstCount, secoundCount] = [secoundCount, firstCount]; // 解构作用于已声明的变量会更好

console.log(firstCount);
console.log(secoundCount);

// 作用于函数参数：
function ss([first, secound]: [string, string]) {
...
let stu = { na: '小明', ag: 12, size: 'bar' };

let { na, ag } = stu;

({ na, ag } = { na: '小红', ag: 11 }); // 对象解构赋值, 这里我们需要用()括号将赋值语句括起来,因为JavaScript经常会以{}为一个代码块处理

let { size, ...anotherObj } = stu; // ...可以创建剩余变量
...
let { na: newNa, ag: newag }: { na: string, ag: number } = stu;

// 指定默认值  默认值可以让属性为undefined时候是用默认值.

function keppWholeObject(wholeObject: { na: string, ag?: number }) {
  let { na, ag = 100 } = wholeObject;
...
function typeFun({ a, b = 10 }: C): void {
  console.log('函数声明', a);
  console.log('函数声明', b);
}
let CParams = { a: 'name' }
...
let newObj = { ...{ a: 'name', b: 'age' }, ...{ c: '123' }, a: '小明' };
